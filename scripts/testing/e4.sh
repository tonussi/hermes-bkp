...
export READ_RATE=$4
export SCENE=$5
export PAYLOAD_SIZE=3
export QTY_ITERATION=200000
...
mkdir -p logs/$EXPERIMENT_NAME/outputs
...
echo $(kubectl get pods -l app=hermes-leader -o=jsonpath='{.items[0].metadata.name}')
kubectl logs $(kubectl get pods -l app=hermes-leader -o=jsonpath='{.items[0].metadata.name}') hermes > logs/$EXPERIMENT_NAME/outputs/leader.log

for i in $(seq 0 1)
do
echo "collecting followers logs..."
echo $(kubectl get pods -l app=hermes-followers -o=jsonpath="{.items[$i].metadata.name}")
kubectl logs $(kubectl get pods -l app=hermes-followers -o=jsonpath="{.items[$i].metadata.name}") hermes > logs/$EXPERIMENT_NAME/outputs/follower-$i.log
done
...
echo "comparing data"
diff logs/$EXPERIMENT_NAME/operations/leader.log logs/$EXPERIMENT_NAME/operations/follower-0.log > logs/$EXPERIMENT_NAME/operations/diff-leader-follower-0.diff
diff logs/$EXPERIMENT_NAME/operations/leader.log logs/$EXPERIMENT_NAME/operations/follower-1.log > logs/$EXPERIMENT_NAME/operations/diff-leader-follower-1.diff
