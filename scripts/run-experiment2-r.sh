...
echo "apply read clients..."
envsubst < $KUBERNETES_DIR/tcp-kv-client-r.yml | kubectl apply -f -

echo "wait job to complete..."
kubectl wait --for=condition=complete --timeout=2m job.batch/tcp-kv-client

TEST=$(expr $N_CLIENTS \* $N_THREADS)-$N_CLIENTS

echo "collecting throughput log..."
kubectl cp $(kubectl get pods -l app=tcp-kv-hashicorp-raft-leader -o=jsonpath='{.items[0].metadata.name}'):/tmp/throughput.log logs/$SCENE/throughput/$TEST.log
